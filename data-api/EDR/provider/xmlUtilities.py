...
def parseCodeRegistryTables(fname, neededCodes, preferredLanguage='en'):
    #
    ...
    for container in root.iter(Containers):
        ...
        containerId = uri[uri.rfind('/') + 1:]
        #
        # Read in only if it is needed for the encoder
        try:
            neededCodes.remove(containerId)
        except ValueError:
            continue
