...
class AutomatedCollectionProvider(BaseProvider):
    ...
    def load_data(self,cid,instance,model):
       ...
       for c in sorted(glob.iglob(self.zarr_store)):
           if os.path.basename(c).lower() == cid.lower():
              c_zarr=c
              print(c)
    
       zarr_obj=xr.open_zarr(c_zarr,consolidated=False)
       return zarr_obj
    ...
    def pt_to_covjson(self,query_dict,coords,qtype):
       ...
       #Create iso dates from metadata and insert as t dimension for covjson
          ...
          self.initial_time=self.find_initial_time(initial_time)
          if isinstance(output['coords'][self.fkey]['data'],list):
             count_for_time=len(output['ranges'][p]['values'])
             f_timedelta=output['coords'][self.fkey]['data']
             iso_time=[]
             for t in f_timedelta:
                time=self.initial_time+t
                time=time.isoformat()
                iso_time.append(time)
             #iso_time=output['coords'][self.fkey]['data']
             output['domain']['axes']['t']={}
             #output['domain']['axes']['t']['values']=output['coords'][self.fkey]['data']
    ...
    def query(self,dataset, qtype, coords, time_range, z_value, params, instance, outputFormat):
       ...
       for idx,dv in enumerate(ds.data_vars):
          self.param_list.append(dv)
       if len(params)==0:
          params=self.param_list
       try:
          initial_time=ds[params[0]].initial_time
          self.initial_time=self.find_initial_time(initial_time)
          start=datetime.datetime.strptime(str(time_range.get_start_date()), "%Y-%m-%dT%H:%M:%SZ")
          end=datetime.datetime.strptime(str(time_range.get_end_date()), "%Y-%m-%dT%H:%M:%SZ")
          start_delta=start-self.initial_time      
          end_delta=end-self.initial_time
       except:
          pass
       for key in dir(ds[params[0]]):
          ...
          if "lat_" in key:
             self.latkey=key
       if self.lonkey=='lon_0':
          ds=ds.assign_coords({'lon_0':ds['lon_0'].values})
       if self.fkey:
          forecast_time_key=self.fkey
          try:
             if start_delta == end_delta:
                arguments={forecast_time_key: start_delta}
             else:
                arguments={forecast_time_key: slice(str(start_delta),str(end_delta))}
             if qtype != 'linestringm' and qtype != 'linestringzm':
                output=ds[params].sel(**arguments)
             else:
                output=ds[params]
          except:
             output=ds[params]
       if z_value and 'linestring' not in qtype:
          if type(z_value)!=int and ',' in z_value:
             ...
             z_value_list=z_list
             z_value=''
             z_value_list = [w.replace("'", '') for w in z_value_list]
             z_value_list = [w.replace('"', '') for w in z_value_list]
             z_arg={self.lvkey: z_value_list}
             output=output.sel(**z_arg)
          elif type(z_value)!=int and '/' in z_value:
             z_list=z_value.split('/')
             try:
                z_0=float(z_list[0]);z_1=float(z_list[1])
             except:
                z_0=int(z_list[0]);z_1=int(z_list[1])
             z_arg={self.lvkey:slice(z_0,z_1)}
             output=output.sel(**z_arg)
          else:
             try:
...
def get_polygon_data(self,dataset, qtype, coords, time_range, z_value, params, instance, outputFormat, output):
          ...
          if outputFormat=="NetCDF":
             #to_netcdf is not working in toolsUI (translating pynio to netcdf)
             #output.to_netcdf(dir_root+'/output'+uuid+'.nc',mode='w',format='NETCDF4_CLASSIC')
             output=output.to_dict()
       ...
       if qtype == 'linestringm' or qtype == 'linestringz' or qtype == 'linestringzm':
          ...
          for m in coords:
              ...
              if qtype=='linestringzm':
                 ...
                 zedd_list.append(zedd)
          if qtype=='linestringm':
             query_args={self.latkey: xr.DataArray(lat_list), self.lonkey: xr.DataArray(lon_list), self.fkey: xr.DataArray(time_list)}
          if qtype=='linestringz':
             query_args={self.latkey: xr.DataArray(lat_list), self.lonkey: xr.DataArray(lon_list), self.lvkey: xr.DataArray(zedd_list)}
          if qtype=='linestringzm':
             query_args={self.latkey: xr.DataArray(lat_list), self.lonkey: xr.DataArray(lon_list), self.lvkey: xr.DataArray(zedd_list), self.fkey: xr.DataArray(time_list)}
          output=output.sel(query_args, method='nearest')
          if outputFormat=="CoverageJSON":
             output=output.to_dict()
             output=self.pt_to_covjson(output,coords,qtype)
...
def get_corridor_data(self,dataset, qtype, coords, time_range, z_value, params, instance, outputFormat, output):
             ...
             if 'linestring' in qtype:
                for waypoints in list_4326:
                   ...
                   if qtype=='linestringz':
                      query_args={self.latkey: xr.DataArray(lat_list), self.lonkey: xr.DataArray(lon_list), self.lvkey: xr.DataArray(zedd_interp_list)}
                   if qtype=='linestringzm':
                      query_args={self.latkey: xr.DataArray(lat_list), self.lonkey: xr.DataArray(lon_list), self.lvkey: xr.DataArray(zedd_interp_list), self.fkey: xr.DataArray(time_interp_list)}
