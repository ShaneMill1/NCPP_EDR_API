...
class AutomatedCollectionProvider(BaseProvider):
    ...
    def load_collection_meta(self,ds,cid):
       ...
       for coor in ds.coords:
          if '_time' in coor:
             ...
             self.ftime_key=coor
          if 'lv_' in coor:
             self.level_type=coor
          if 'lat' in coor:
             self.latkey=coor
          if 'lon' in coor:
             self.lonkey=coor
       if hasattr(ds[first_dv],'GRIB_typeOfLevel'):
    ...
    def pt_to_covjson(self,query_dict,coords,qtype):
       ...
       del output['coords'][self.latkey]['data']
       del output['coords'][self.lonkey]['data']
       output['ranges']={}
       #find dim with "lv" and create the "z" dimension
       for key in output['coords']:
          if key != self.latkey and key != self.lonkey and key != 'step' and 'time' not in key and key !='spatial_ref' and key!=self.fkey:
             z_key=key
...
def get_point_data(self,dataset, qtype, coords, time_range, z_value, params, instance, outputFormat, output):
   ...
   lonattr=self.lonkey
   output=output.assign_coords({self.lonkey: getattr(output,self.lonkey) - 360})
   output=output.rio.set_spatial_dims(self.lonkey,self.lonkey,inplace=True)
...
def get_polygon_data(self,dataset, qtype, coords, time_range, z_value, params, instance, outputFormat, output):
          ...
          output=output.assign_coords({self.lonkey: getattr(output,self.lonkey) - 360})
          output_bdl=output
          output=xr.concat([output_bdl,output_adl],dim=self.lonkey)
...
def convert_corridor_to_cf(self,output_list):
   ...
   for idx,traj_output in enumerate(output_list):
      ...
      traj_output=traj_output.rename({self.latkey:self.latkey+'_'++str(idx)})
      traj_output=traj_output.rename({self.lonkey:self.lonkey+'_'+str(idx)})
...
def export_geotiff(self,output):
   ...
   for dims in output.dims:
      if '_time' in dims or (dims != self.latkey and dims != self.lonkey):
   ...
   if len(dim_tracker)==0:
      ...
      if len(dim_tracker.keys())==1:
         if self.fkey in list(dim_tracker.keys())[0]:
