...
class AutomatedCollectionProvider(BaseProvider):
    ...
    def query(self,dataset, qtype, coords, time_range, z_value, params, instance, outputFormat):
       ...
       for idx,dv in enumerate(ds.data_vars):
          param_list.append(dv)
       if len(params)==0:
          params=param_list
       try:
       ...
       if self.fkey:
          ...
          try:
             ...
             output=ds[params]
       if z_value:
          if self.lvkey not in output.coords:
             output=output.assign_coords({self.lvkey:output[self.lvkey]})
             output[self.lvkey]=output[self.lvkey].data+1
          if 'e' in z_value:
          ...
          if '-' in z_value:
             ...
             for idx,l in enumerate(levels_meta,1):
                if l == z_value:
                   z_value=idx
                   z_arg={self.lvkey: z_value}
          ...
          if '/' in str(z_value):
             try:
                ...
                pass
          if '/' in str(z_value) and '-' in str(z_value):
             z_value_list=list()
             url=flask.request.url
             url_component=url.split('?')
             url_slashes=url_component[0].split('/')
             ...
             for z in zs:
                for idx,l in enumerate(levels_meta):
                   if l == z:
                      z_value_list.append(idx)
                   else:
                      pass
             z_arg={self.lvkey: slice(z_value_list[0]+1,z_value_list[1]+1)}
             output=output.sel(**z_arg)
             for lv in output[self.lvkey]:
                lv_list.append(levels_meta[int(lv)-1])
             print('test')
          if '[' in z_value and ']' in z_value:
             z_value=z_value.replace('[','')
             z_value=z_value.replace(']','')
             z_list=z_value.split(',')
             output=output.sel({self.lvkey:z_list})
          else:
             try:
                try:
                   z_value=int(z_value)
       ...
       if qtype=='polygon':
          ...
          if outputFormat=="GRIB":
             ...
             convert_to_grib.create_grib(output,grib_template,lv_list)
             output={'result': 'grib file should display for download'}
             return flask.send_from_directory('/data/','output.grb',as_attachment=True), dir_root+'/output.grb'
