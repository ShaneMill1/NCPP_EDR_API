...
def parameter_map(pt, p_name, config):
    if config is not None:
        if type(config['datasets']['taf']['parameters'][p_name][0]) is str:
            mp_ = MetadataProvider(config)
            parts = config['datasets']['taf']['parameters'][p_name][0].split('/')
            pt['parameters'][p_name] =  mp_.get_buf4_detail(parts[2],parts[3],parts[4],None)
        else:
            pt['parameters'][p_name] = config['datasets']['taf']['parameters'][p_name][0]

def set_parameter_vals(pt, p_names, config):

    if len(p_names) == 0:
        p_names = config['datasets']['taf']['parameters']
    for p in p_names:
...
def set_range_vals(pt, data, t_key, p_names, config):
    ...
    for m in data:
        for p in p_names:
            val = get_item(m, config['datasets']['taf']['parameters'][p][2])
            if val is not None:
                if config['datasets']['taf']['parameters'][p][3] == 'float':
                    pt['ranges'][p]['values'].append(float(val))
                elif config['datasets']['taf']['parameters'][p][3] == 'int':
...
def get_item(taf, item):
    try:
        if item == 'wind_speed':     
            return taf.wind_speed.value()
        elif item == 'wind_gust':     
            return taf.wind_gust.value()
        elif item == 'wind_dir':     
            return taf.wind_dir.value()
        elif item == 'vis':     
            return taf.vis.value()
        elif item == 'sky':     
            return taf.sky.value()
        elif item == 'weather':     
            return taf.weather.value()
        elif item == 'raw_ob':     
            return taf.code        
        elif item == 'icao_id':     
            return taf.station_id
        else:
            return None
