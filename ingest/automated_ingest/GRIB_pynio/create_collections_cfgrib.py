...
def create_collections(download_grib_location,zarr_output_location):
   ...
   col_name_dict={}
   fs_list=list()
   convert_to_zarr(download_grib_location+'/combined_data/combined.grb',zarr_output_location,col_name_dict,fs_list)
   return
...
def convert_to_zarr(f,zarr_output_location,col_name_dict,fs_list):
   ...
   for idxx,ds in enumerate(ds_lists):
      ...
      if col_name_desc in f_name_list:
         try:
            ds.to_zarr(fsspec.get_mapper(zarr_output_location+col_name_desc,client_kwargs={'region_name':'us-west-2'}),mode='a',append_dim='valid_time')
            print(col_name_desc+' appended')
         except:
            print('--------WARNING----------'+col_name_desc+' FAILED TO EXPORT TO ZARR')
      else:
         ds.to_zarr(fsspec.get_mapper(zarr_output_location+col_name_desc,client_kwargs={'region_name':'us-west-2'}),mode='w')
